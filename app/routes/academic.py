...
def delete_session(session_id: int, db: Session = Depends(get_db), current_user=Depends(is_principal_or_above)):
    ...
    class_ids = [row[0] for row in db.query(Class.id).filter(Class.session_id == session_id).all()]
    if db.query(FeeStructure.id).filter(FeeStructure.session_id == session_id).first():
        raise HTTPException(status_code=400, detail="Cannot delete: this session has fee structures. Remove them first.")
    if db.query(Invoice.id).filter(Invoice.session_id == session_id).first():
        raise HTTPException(status_code=400, detail="Cannot delete: this session has invoices or payments recorded.")
    if db.query(Result.id).filter(Result.session_id == session_id).first():
        raise HTTPException(status_code=400, detail="Cannot delete: this session has recorded results.")
    if db.query(ReportCardMeta.id).filter(ReportCardMeta.session_id == session_id).first():
        raise HTTPException(status_code=400, detail="Cannot delete: this session has report card records.")
    if class_ids and db.query(Student.id).filter(Student.current_class_id.in_(class_ids)).first():
        raise HTTPException(status_code=400, detail="Cannot delete: students are still assigned to classes in this session.")
    if class_ids and db.query(Attendance.id).filter(Attendance.class_id.in_(class_ids)).first():
        raise HTTPException(status_code=400, detail="Cannot delete: attendance records exist for classes in this session.")

    if class_ids:
        db.query(TeacherSubjectClass).filter(TeacherSubjectClass.class_id.in_(class_ids)).delete(synchronize_session=False)
