...
def _student_target_group(db: Session, student: Student, session: Optional[AcademicSession]) -> FeeStructureTarget:
    if not session or not student.enrollment_date:
        return FeeStructureTarget.RETURNING
    if session.start_date <= student.enrollment_date <= session.end_date:
        return FeeStructureTarget.NEW_INTAKE
    return FeeStructureTarget.RETURNING


def get_fee_structure_for_student(db: Session, student: Student, session_id: int, term_id: int) -> Optional[FeeStructure]:
    session = db.query(AcademicSession).filter(AcademicSession.id == session_id).first()
    target_group = _student_target_group(db, student, session)
    exact = db.query(FeeStructure).filter(
        FeeStructure.class_id == student.current_class_id,
        FeeStructure.session_id == session_id,
        FeeStructure.term_id == term_id,
        FeeStructure.target_group == target_group,
    ).first()
    if exact:
        return exact
    return db.query(FeeStructure).filter(
        ...
        FeeStructure.session_id == session_id,
        FeeStructure.term_id == term_id,
        FeeStructure.target_group == FeeStructureTarget.ALL,
    ).first()
