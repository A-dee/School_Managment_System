...
def is_announcement_manager(current_user: User = Depends(get_current_user)) -> User:
    """SUPER_ADMIN + ADMIN (Proprietor + Principal) only."""
    if current_user.role not in (UserRole.SUPER_ADMIN, UserRole.ADMIN):
        raise HTTPException(status_code=403, detail="Announcement management access required")
    return current_user
