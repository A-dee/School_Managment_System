...
import {
  ...
  MessageSquare, Megaphone, Bell, ChevronRight, Calendar,
} from "lucide-react";
...
export default function ParentDashboard() {
  ...
  const typeColors: Record<string, string> = { NOTICE: "#6366f1", EVENT: "#f59e0b", HOLIDAY: "#22c55e" };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="t-card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/parent/announcements" className="flex items-center gap-1 text-xs" style={{ color: "var(--accent)", textDecoration: "none" }}>
              View all <ChevronRight size={12} />
            </Link>
          </div>

          {loading ? (
            <div className="flex justify-center py-6"><div className="t-spinner" /></div>
            ...
            <div className="flex flex-col gap-3">
              {announcements.map((a: any) => (
                <div key={a.id} style={{ padding: "10px 12px", borderRadius: 9, border: "1px solid var(--border)", borderLeft: `3px solid ${typeColors[a.type] || "var(--accent)"}` }}>
                  {a.event_date && (
                    <div className="flex items-center gap-1 mb-1" style={{ color: typeColors[a.type], fontSize: "0.68rem", fontWeight: 700 }}>
                      <Calendar size={10} />
                      {new Date(a.event_date).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
                    </div>
                  )}
                  <p className="t-text-primary font-semibold" style={{ fontSize: "0.8rem", marginBottom: 3 }}>{a.title}</p>
                  <p className="t-text-secondary" style={{ fontSize: "0.72rem", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {a.message}
                  </p>
                  <p className="t-text-secondary" style={{ fontSize: "0.65rem", marginTop: 4 }}>
